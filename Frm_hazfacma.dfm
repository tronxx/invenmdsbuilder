object Form_hazfacma: TForm_hazfacma
  Left = 255
  Top = 139
  Width = 662
  Height = 559
  Caption = 'Captura de Facturas Mayoreo'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefaultPosOnly
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox2: TGroupBox
    Left = 0
    Top = 270
    Width = 646
    Height = 87
    Align = alTop
    TabOrder = 1
    inline Frame_datosrenfacma1: TFrame_datosrenfacma
      Left = 2
      Top = 15
      Width = 642
      Align = alTop
      inherited edt_codigo: TEdit
        ReadOnly = True
        OnExit = Frame_datosrenfacma1edt_codigoExit
        OnKeyDown = Frame_datosrenfacma1edt_codigoKeyDown
      end
      inherited edt_canti: TdxCurrencyEdit
        StoredValues = 1
      end
      inherited edt_preciou: TdxCurrencyEdit
        StoredValues = 1
      end
      inherited edt_pdsc: TdxCurrencyEdit
        StoredValues = 1
      end
      inherited btn_series: TButton
        Caption = '...'
        Visible = False
      end
      inherited edt_piva: TdxCurrencyEdit
        StoredValues = 1
      end
    end
  end
  object GroupBox3: TGroupBox
    Left = 0
    Top = 398
    Width = 646
    Height = 122
    Align = alClient
    Caption = 'Re&nglones'
    TabOrder = 2
    object dbgrd_rengnes: TdxDBGrid
      Left = 2
      Top = 15
      Width = 642
      Height = 105
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'IDRENFACMA'
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alClient
      PopupMenu = pop_menuren
      TabOrder = 0
      OnKeyDown = dbgrd_rengnesKeyDown
      DataSource = dts_renfacma
      Filter.Criteria = {00000000}
      OptionsBehavior = [edgoAutoSort, edgoDragScroll, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
      OnCustomDrawCell = dbgrd_rengnesCustomDrawCell
      object dbgrd_rengnesCODIGO: TdxDBGridMaskColumn
        Caption = 'Código'
        Width = 121
        BandIndex = 0
        RowIndex = 0
        FieldName = 'CODIGO'
      end
      object dbgrd_rengnesCONCEPTO: TdxDBGridMaskColumn
        Caption = 'Descripción'
        Width = 253
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCRI'
      end
      object dbgrd_rengnesUNIDS: TdxDBGridMaskColumn
        Alignment = taRightJustify
        Caption = 'Cant.'
        Width = 37
        BandIndex = 0
        RowIndex = 0
        FieldName = 'UNIDS'
      end
      object dbgrd_rengnesPRECIOU: TdxDBGridCurrencyColumn
        Alignment = taRightJustify
        Caption = 'Precio Unit.'
        Width = 77
        BandIndex = 0
        RowIndex = 0
        FieldName = 'PRECIOU'
      end
      object dbgrd_rengnesPDSC: TdxDBGridMaskColumn
        Alignment = taRightJustify
        Caption = '%Dscto'
        Width = 52
        BandIndex = 0
        RowIndex = 0
        FieldName = 'PORDSC'
      end
      object dbgrd_rengnesIMPORTE: TdxDBGridCurrencyColumn
        Alignment = taRightJustify
        Caption = 'Importe'
        Width = 82
        BandIndex = 0
        RowIndex = 0
        FieldName = 'IMPORTE'
      end
    end
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 54
    Width = 646
    Height = 216
    Align = alTop
    TabOrder = 0
    inline Frame_datosfacma1: TFrame_datosfacma
      Left = 2
      Top = 15
      Width = 642
      Height = 199
      Align = alClient
      inherited edt_numfac: TdxCurrencyEdit
        OnKeyDown = Frame_datosfacma1edt_numfacKeyDown
        StoredValues = 1
      end
      inherited edt_npagos: TdxCurrencyEdit
        StoredValues = 1
      end
      inherited edt_pzoxlet: TdxCurrencyEdit
        StoredValues = 1
      end
      inherited edt_import: TdxDBCurrencyEdit
        DataSource = dts_facturma
        StoredValues = 1
      end
      inherited edt_descto: TdxDBCurrencyEdit
        DataSource = dts_facturma
        StoredValues = 1
      end
      inherited edt_subtot: TdxDBCurrencyEdit
        DataSource = dts_facturma
        StoredValues = 1
      end
      inherited edt_iva: TdxDBCurrencyEdit
        DataSource = dts_facturma
        StoredValues = 1
      end
      inherited edt_total: TdxDBCurrencyEdit
        DataSource = dts_facturma
        StoredValues = 1
      end
    end
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 646
    Height = 54
    ButtonHeight = 49
    ButtonWidth = 49
    Caption = 'ToolBar1'
    Images = dm.ImageList1
    TabOrder = 3
    object SpeedButton5: TSpeedButton
      Left = 0
      Top = 2
      Width = 49
      Height = 49
      Action = nuevo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF0000FFFF00}
      Layout = blGlyphTop
    end
    object SpeedButton3: TSpeedButton
      Left = 49
      Top = 2
      Width = 49
      Height = 49
      Action = borra
      Glyph.Data = {
        
        FF00FF00FF00FF00FF0000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton9: TSpeedButton
      Left = 98
      Top = 2
      Width = 49
      Height = 49
      Action = impritext
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton2: TSpeedButton
      Left = 147
      Top = 2
      Width = 49
      Height = 49
      Action = cierra_entrada
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton14: TSpeedButton
      Left = 196
      Top = 2
      Width = 49
      Height = 49
      Action = modifica
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton8: TSpeedButton
      Left = 245
      Top = 2
      Width = 49
      Height = 49
      Action = graba_permision_precio
      Glyph.Data = {
        
        FF00FFFFFF000000FF000000FF00FFFFFF000000FF000000FF00}
      Layout = blGlyphTop
    end
    object ToolButton3: TToolButton
      Left = 294
      Top = 2
      Width = 8
      Caption = 'ToolButton3'
      ImageIndex = 5
      Style = tbsSeparator
    end
    object SpeedButton4: TSpeedButton
      Left = 302
      Top = 2
      Width = 49
      Height = 49
      Action = agreren
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton11: TSpeedButton
      Left = 351
      Top = 2
      Width = 49
      Height = 49
      Action = primero
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton10: TSpeedButton
      Left = 400
      Top = 2
      Width = 49
      Height = 49
      Action = anterior
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton7: TSpeedButton
      Left = 449
      Top = 2
      Width = 49
      Height = 49
      Action = siguiente
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton6: TSpeedButton
      Left = 498
      Top = 2
      Width = 49
      Height = 49
      Action = ultimo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF000000000000000000}
      Layout = blGlyphTop
    end
    object SpeedButton1: TSpeedButton
      Left = 547
      Top = 2
      Width = 49
      Height = 49
      Action = WindowClose1
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 357
    Width = 646
    Height = 41
    Align = alTop
    TabOrder = 4
    object SpeedButton12: TSpeedButton
      Left = 5
      Top = -8
      Width = 102
      Height = 49
      Action = agreren
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
    object SpeedButton13: TSpeedButton
      Left = 141
      Top = -8
      Width = 102
      Height = 49
      Action = borraren
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
  end
  object ActionList1: TActionList
    Images = dm.ImageList1
    Left = 280
    Top = 256
    object nuevo: TAction
      Caption = '&Nuevo'
      ImageIndex = 0
      ShortCut = 16462
      OnExecute = nuevoExecute
    end
    object modifica: TAction
      Caption = '&Modifica'
      Hint = 'Modifica'
      ImageIndex = 1
      ShortCut = 16461
      OnExecute = modificaExecute
    end
    object borra: TAction
      Caption = 'Borra'
      Hint = 'Borra'
      ImageIndex = 2
      ShortCut = 16450
      OnExecute = borraExecute
    end
    object grabar: TAction
      Caption = 'Grabar'
      Enabled = False
      ImageIndex = 9
      ShortCut = 16455
    end
    object cancelar: TAction
      Caption = 'Cancelar'
      ImageIndex = 11
      ShortCut = 16460
      OnExecute = cancelarExecute
    end
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = 'Salir'
      ImageIndex = 3
    end
    object primero: TAction
      Caption = 'primero'
      Hint = 'Selecciona la Primer Marca'
      ImageIndex = 5
      ShortCut = 49232
      OnExecute = primeroExecute
    end
    object anterior: TAction
      Caption = 'Anterior'
      Hint = 'Selecciona la Anterior Marca'
      ImageIndex = 6
      ShortCut = 49217
      OnExecute = anteriorExecute
    end
    object siguiente: TAction
      Caption = 'Siguiente'
      Hint = 'Selecciona la Siguiente Marca'
      ImageIndex = 8
      ShortCut = 49235
      OnExecute = siguienteExecute
    end
    object ultimo: TAction
      Caption = 'Ultimo'
      Hint = 'Selecciona la Ultima Marca'
      ImageIndex = 7
      ShortCut = 49237
      OnExecute = ultimoExecute
    end
    object agreren: TAction
      Caption = 'A&grega.Reng'
      ImageIndex = 13
      OnExecute = agrerenExecute
    end
    object cierraren: TAction
      Caption = 'Afectar Renglón'
      ImageIndex = 15
      ShortCut = 16454
    end
    object borraren: TAction
      Caption = 'Eliminar Renglón'
      ImageIndex = 17
      ShortCut = 16453
      OnExecute = borrarenExecute
    end
    object exportdbgrid: TAction
      Caption = 'Exportar a ...'
      ImageIndex = 19
      OnExecute = exportdbgridExecute
    end
    object cierra_entrada: TAction
      Caption = 'Cerrar'
      ImageIndex = 15
      OnExecute = cierra_entradaExecute
    end
    object impritext: TAction
      Caption = 'Imprimir'
      ImageIndex = 21
      ShortCut = 16464
      OnExecute = impritextExecute
    end
    object despliega_menu: TAction
      Caption = 'Menu'
      ShortCut = 49229
      OnExecute = despliega_menuExecute
    end
    object tomar_salida_mayoreo: TAction
      Caption = 'Salida Mayoreo'
      ImageIndex = 23
    end
    object graba_uuid: TAction
      Caption = 'Grabar Uuid'
      OnExecute = graba_uuidExecute
    end
    object graba_permision_precio: TAction
      Caption = 'Precio'
      ImageIndex = 29
      OnExecute = graba_permision_precioExecute
    end
  end
  object qry_renfacma: TQuery
    CachedUpdates = True
    AfterScroll = qry_renfacmaAfterScroll
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'select *'
      'from renfacma a'
      'where idfactur = :IDFACTURMA order by consec')
    UpdateObject = uqry_renfacma
    Left = 375
    Top = 345
    ParamData = <
      item
        DataType = ftInteger
        Name = 'IDFACTURMA'
        ParamType = ptUnknown
      end>
  end
  object dts_renfacma: TDataSource
    AutoEdit = False
    DataSet = qry_renfacma
    Left = 375
    Top = 374
  end
  object qry_sigfacma: TQuery
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'select max (num) as sigfacma from facturma'
      'where serie = :SERIEFAC and cia = :CIA')
    Left = 407
    Top = 345
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'SERIEFAC'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object qry_facturma: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'select seriefac, NUM, mayoris, refer, nomay, dir, rfc, status,'
      
        'fecha,vence,importe,descu,importe - descu as neto, iva, total, c' +
        'ia,'
      'npagos, plazo, tipago,'
      'pdsc, mayomen, idfactur, tipofac, iduuid'
      'from facturma'
      
        'where seriefac = :SERIE and tipofac = :TIPOFAC and num = :NUMERO' +
        ' and cia = :CIA'
      ''
      ' '
      ' '
      ' '
      ' ')
    UpdateObject = uqry_facturma
    Left = 439
    Top = 345
    ParamData = <
      item
        DataType = ftString
        Name = 'SERIE'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'TIPOFAC'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'NUMERO'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object uqry_facturma: TUpdateSQL
    ModifySQL.Strings = (
      'update facturma'
      'set'
      '  MAYORIS = :MAYORIS,'
      '  REFER = :REFER,'
      '  NOMAY = :NOMAY,'
      '  DIR = :DIR,'
      '  RFC = :RFC,'
      '  STATUS = :STATUS,'
      '  FECHA = :FECHA,'
      '  VENCE = :VENCE,'
      '  IMPORTE = :IMPORTE,'
      '  DESCU = :DESCU,'
      '  NETO = :NETO,'
      '  IVA = :IVA,'
      '  TOTAL = :TOTAL,'
      '  CIA = :CIA,'
      '  NPAGOS = :NPAGOS,'
      '  PLAZO = :PLAZO,'
      '  TIPAGO = :TIPAGO,'
      '  PDSC = :PDSC,'
      '  MAYOMEN = :MAYOMEN,'
      '  IDUUID = :IDUUID'
      'where'
      '  IDFACTUR = :OLD_IDFACTUR')
    InsertSQL.Strings = (
      'insert into facturma'
      
        '  (SERIEFAC, NUM, MAYORIS, REFER, NOMAY, DIR, RFC, STATUS, FECHA' +
        ', '
      'VENCE, '
      
        '   IMPORTE, DESCU, NETO, IVA, TOTAL, CIA, NPAGOS, PLAZO, TIPAGO,' +
        ' PDSC, '
      '   MAYOMEN, IDFACTUR, TIPOFAC,   IDUUID'
      ')'
      'values'
      
        '  (:SERIEFAC, :NUM, :MAYORIS, :REFER, :NOMAY, :DIR, :RFC, :STATU' +
        'S, :FECHA, '
      
        '   :VENCE, :IMPORTE, :DESCU, :NETO, :IVA, :TOTAL, :CIA, :NPAGOS,' +
        ' :PLAZO, '
      '   :TIPAGO, :PDSC, :MAYOMEN, :IDFACTUR, :TIPOFAC,  :IDUUID )')
    DeleteSQL.Strings = (
      'delete from facturma'
      'where'
      '  IDFACTUR = :OLD_IDFACTUR')
    Left = 438
    Top = 375
  end
  object qry_ultfol: TQuery
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'select IFNULL(min(folio), 0) as ultfol, count(*) as exist'
      ' from movart'
      
        'where idart = :IDART and  idalm = :IDALM and salio = '#39'N'#39' and fec' +
        'ha <= :FECHA'
      'and cia = :CIA')
    Left = 407
    Top = 376
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDART'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'IDALM'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'FECHA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object qry_conse: TQuery
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'select IFNULL(max(consec),0) as ultren'
      'from renfacma where idfactur = :IDFACTURMA'
      ' ')
    Left = 503
    Top = 346
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDFACTURMA'
        ParamType = ptUnknown
      end>
  end
  object qry_borraren: TQuery
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'delete from renentra where idrenentra = :IDRENENTRA')
    Left = 503
    Top = 378
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDRENENTRA'
        ParamType = ptUnknown
      end>
  end
  object uqry_renfacma: TUpdateSQL
    ModifySQL.Strings = (
      'update renfacma'
      'set'
      '  FACTUR = :FACTUR,'
      '  CONSEC = :CONSEC,'
      '  CODIGO = :CODIGO,'
      '  DESCRI = :DESCRI,'
      '  UNIDS = :UNIDS,'
      '  PRECIOU = :PRECIOU,'
      '  IMPORTE = :IMPORTE,'
      '  PORDSC = :PORDSC,'
      '  DESCU = :DESCU,'
      '  PORIVA = :PORIVA,'
      '  IVA = :IVA,'
      '  TOTAL = :TOTAL,'
      '  COSTOU = :COSTOU,'
      '  CIA = :CIA,'
      '  IDRENFACMA = :IDRENFACMA,'
      '  IDFACTUR = :IDFACTUR'
      'where'
      '  IDRENFACMA = :OLD_IDRENFACMA')
    InsertSQL.Strings = (
      'insert into renfacma'
      
        '  (FACTUR, CONSEC, CODIGO, DESCRI, UNIDS, PRECIOU, IMPORTE, PORD' +
        'SC, '
      'DESCU, '
      '   PORIVA, IVA, TOTAL, COSTOU, CIA, IDRENFACMA, IDFACTUR)'
      'values'
      
        '  (:FACTUR, :CONSEC, :CODIGO, :DESCRI, :UNIDS, :PRECIOU, :IMPORT' +
        'E, '
      ':PORDSC, '
      
        '   :DESCU, :PORIVA, :IVA, :TOTAL, :COSTOU, :CIA, :IDRENFACMA, :I' +
        'DFACTUR)')
    DeleteSQL.Strings = (
      'delete from renfacma'
      'where'
      '  IDRENFACMA = :OLD_IDRENFACMA')
    Left = 376
    Top = 317
  end
  object dts_facturma: TDataSource
    AutoEdit = False
    DataSet = qry_facturma
    Left = 436
    Top = 315
  end
  object pop_menuren: TPopupMenu
    Left = 72
    Top = 286
    object EliminarRengln1: TMenuItem
      Action = borraren
    end
    object AgregaReng1: TMenuItem
      Action = cierraren
    end
    object Exportara1: TMenuItem
      Action = exportdbgrid
    end
  end
  object qry_mayoris: TQuery
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select * from mayoris where cia = :CIA and status = '#39'A'#39
      'order by codigo'
      ' '
      ' ')
    Left = 161
    Top = 347
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object dts_mayoris: TDataSource
    DataSet = qry_mayoris
    Left = 161
    Top = 376
  end
  object qry_preciofac: TQuery
    DatabaseName = 'manvehi'
    Filtered = True
    SQL.Strings = (
      'select sum(importe - descu) as preciofac  from renfacma'
      'where idfacturma = :IDFACTURMA and idart = :IDART')
    Left = 191
    Top = 408
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDFACTURMA'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'IDART'
        ParamType = ptUnknown
      end>
  end
  object qry_venfacma: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select idvenfacma, idfacturma, letra, vencto from venfacma '
      'where idfacturma = :IDFACTURMA order by letra'
      '')
    UpdateObject = uqry_venfacma
    Left = 65
    Top = 349
    ParamData = <
      item
        DataType = ftInteger
        Name = 'IDFACTURMA'
        ParamType = ptUnknown
      end>
  end
  object uqry_venfacma: TUpdateSQL
    ModifySQL.Strings = (
      'update venfacma'
      'set'
      '  LETRA = :LETRA,'
      '  VENCTO = :VENCTO'
      'where'
      '  IDVENFACMA = :OLD_IDVENFACMA')
    InsertSQL.Strings = (
      'insert into venfacma'
      '  (IDVENFACMA, IDFACTURMA, LETRA, VENCTO)'
      'values'
      '  (:IDVENFACMA, :IDFACTURMA, :LETRA, :VENCTO)')
    DeleteSQL.Strings = (
      'delete from venfacma'
      'where'
      '  IDVENFACMA = :OLD_IDVENFACMA')
    Left = 66
    Top = 320
  end
  object qry_movmay: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select'
      'mayoris,docto,pagare,conse,fecha,vence,concep,coa,importe,'
      'saldo,cia,tipago,idconcep,fecsal,idmov,tipofac'
      'from movmay2'
      'where mayoris = :MAYORIS and docto = :DOCTO and'
      '(  coa = :CARGOS or coa = :ABONOS ) '
      'and cia = :CIA'
      ' ')
    Left = 567
    Top = 347
    ParamData = <
      item
        DataType = ftString
        Name = 'MAYORIS'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'DOCTO'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'CARGOS'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'ABONOS'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object uqry_movmay: TUpdateSQL
    ModifySQL.Strings = (
      'update movmay2'
      'set'
      '  MAYORIS = :MAYORIS,'
      '  DOCTO = :DOCTO,'
      '  PAGARE = :PAGARE,'
      '  CONSE = :CONSE,'
      '  FECHA = :FECHA,'
      '  VENCE = :VENCE,'
      '  CONCEP = :CONCEP,'
      '  COA = :COA,'
      '  IMPORTE = :IMPORTE,'
      '  SALDO = :SALDO,'
      '  CIA = :CIA,'
      '  TIPAGO = :TIPAGO,'
      '  IDCONCEP = :IDCONCEP,'
      '  FECSAL = :FECSAL,'
      '  IDMOV = :IDMOV,'
      '  TIPOFAC = :TIPOFAC'
      'where'
      '  IDMOV = :OLD_IDMOV')
    InsertSQL.Strings = (
      'insert into movmay2'
      '  (MAYORIS, DOCTO, PAGARE, CONSE, FECHA, VENCE, CONCEP, COA, '
      'IMPORTE, SALDO, '
      '   CIA, TIPAGO, IDCONCEP, FECSAL, IDMOV, TIPOFAC)'
      'values'
      
        '  (:MAYORIS, :DOCTO, :PAGARE, :CONSE, :FECHA, :VENCE, :CONCEP, :' +
        'COA, '
      ':IMPORTE, '
      '   :SALDO, :CIA, :TIPAGO, :IDCONCEP, :FECSAL, :IDMOV, :TIPOFAC)')
    DeleteSQL.Strings = (
      'delete from movmay2'
      'where'
      '  IDMOV = :OLD_IDMOV')
    Left = 568
    Top = 379
  end
  object qry_borraseries: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'delete from seriefacma'
      'where idfactur = :IDFACTURMA'
      ' ')
    Left = 129
    Top = 315
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDFACTURMA'
        ParamType = ptUnknown
      end>
  end
  object uqry_seriefacma: TUpdateSQL
    ModifySQL.Strings = (
      'update seriefacma'
      'set'
      '  FACTUR = :FACTUR,'
      '  RENGLON = :RENGLON,'
      '  CONSEC = :CONSEC,'
      '  CODINV = :CODINV,'
      '  SERIE = :SERIE,'
      '  CIA = :CIA,'
      '  IDSERIEFACMA = :IDSERIEFACMA,'
      '  IDFACTUR = :IDFACTUR,'
      '  IDRENFACMA = :IDRENFACMA'
      'where'
      '  IDSERIEFACMA = :OLD_IDSERIEFACMA')
    InsertSQL.Strings = (
      'insert into seriefacma'
      
        '  (FACTUR, RENGLON, CONSEC, CODINV, SERIE, CIA, IDSERIEFACMA, ID' +
        'FACTUR, '
      '   IDRENFACMA)'
      'values'
      
        '  (:FACTUR, :RENGLON, :CONSEC, :CODINV, :SERIE, :CIA, :IDSERIEFA' +
        'CMA, :IDFACTUR, '
      '   :IDRENFACMA)')
    DeleteSQL.Strings = (
      'delete from seriefacma'
      'where'
      '  IDSERIEFACMA = :OLD_IDSERIEFACMA')
    Left = 96
    Top = 315
  end
  object qry_seriefacma: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select'
      
        'factur,renglon,consec,codinv,serie,cia,idseriefacma,idfactur,idr' +
        'enfacma'
      'from seriefacma'
      'where idrenfacma = :IDRENFACMA order by serie')
    UpdateObject = uqry_seriefacma
    Left = 97
    Top = 344
    ParamData = <
      item
        DataType = ftInteger
        Name = 'IDRENFACMA'
        ParamType = ptUnknown
      end>
  end
  object dts_seriefacma: TDataSource
    DataSet = qry_seriefacma
    Left = 97
    Top = 373
  end
  object qry_datospago: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      
        'select a.idrelac, a.idmayoris, a.mayoris, a.idtiporel, a.iddato1' +
        ', a.iddato2, a.cia,'
      'b.concep as tipopago, c.concep as numcta'
      'from may_relac a join inv_conceps b on a.iddato1 = b.idconcep'
      'join inv_conceps c on a.iddato2 = c.idconcep'
      'where a.idmayoris = :IDFAC and a.cia = :CIA'
      '')
    UpdateObject = uqry_datospago
    Left = 586
    Top = 284
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDFAC'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object uqry_datospago: TUpdateSQL
    ModifySQL.Strings = (
      'update may_relac'
      'set'
      '  IDRELAC = :IDRELAC,'
      '  IDMAYORIS = :IDMAYORIS,'
      '  MAYORIS = :MAYORIS,'
      '  IDTIPOREL = :IDTIPOREL,'
      '  IDDATO1 = :IDDATO1,'
      '  IDDATO2 = :IDDATO2,'
      '  CIA = :CIA'
      'where'
      '  IDRELAC = :OLD_IDRELAC')
    InsertSQL.Strings = (
      'insert into may_relac'
      
        '  (IDRELAC, IDMAYORIS, MAYORIS, IDTIPOREL, IDDATO1, IDDATO2, CIA' +
        ')'
      'values'
      
        '  (:IDRELAC, :IDMAYORIS, :MAYORIS, :IDTIPOREL, :IDDATO1, :IDDATO' +
        '2, :CIA)')
    DeleteSQL.Strings = (
      'delete from may_relac'
      'where'
      '  IDRELAC = :OLD_IDRELAC')
    Left = 584
    Top = 312
  end
  object PopupMenu1: TPopupMenu
    Left = 304
    Top = 423
    object Facturas1: TMenuItem
      Caption = 'Facturas'
      object Nuevo1: TMenuItem
        Action = nuevo
      end
      object Modifica1: TMenuItem
        Action = modifica
      end
      object Borra1: TMenuItem
        Action = borra
      end
      object Cerrar1: TMenuItem
        Action = cierra_entrada
      end
      object Anterior1: TMenuItem
        Action = anterior
      end
      object Siguiente1: TMenuItem
        Action = siguiente
      end
      object primero1: TMenuItem
        Action = primero
      end
      object Ultimo1: TMenuItem
        Action = ultimo
      end
      object Imprimir1: TMenuItem
        Action = impritext
      end
      object GrabarUuid1: TMenuItem
        Action = graba_uuid
        ShortCut = 49223
      end
      object Cancelar1: TMenuItem
        Action = cancelar
      end
    end
    object Renglones1: TMenuItem
      Caption = 'Renglones'
      object Nuevo2: TMenuItem
        Action = nuevo
      end
      object Borra2: TMenuItem
        Action = borra
      end
    end
  end
  object qry_datosmayoris: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      
        'select a.idrelac, a.idmayoris, a.mayoris, a.idtiporel, a.iddato1' +
        ', a.iddato2, a.cia,'
      'b.concep as tipopago, c.concep as numcta'
      'from may_relac a join inv_conceps b on a.iddato1 = b.idconcep'
      'join inv_conceps c on a.iddato2 = c.idconcep'
      'where a.idmayoris = -1 and mayoris = :MAYORIS and a.cia = :CIA'
      '')
    Left = 618
    Top = 284
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'MAYORIS'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
end
