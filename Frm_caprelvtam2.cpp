...
TForm_caprelvtam2 *Form_caprelvtam2;
//---------------------------------------------------------------------------
__fastcall TForm_caprelvtam2::TForm_caprelvtam2(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------

void __fastcall TForm_caprelvtam2::borraExecute(TObject *Sender)
{
  bool si_borrar = true;
  int idrelvtas_z = 0;
  ...
  if(Application->MessageBox(mensaje_z.c_str(), titmsg_z.c_str(), MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2) == IDYES) {
    ...
    for(qry_renentra->First(); !qry_renentra->Eof; qry_renentra->First() ) {
        qry_renentra->Delete();
    }
    ...
    try {
      ...
      if (qry_renentra->UpdatesPending)        qry_renentra->ApplyUpdates();
      dm->manvehi->Commit();
    } catch (Exception &E) {
      dm->manvehi->Rollback();
...
//---------------------------------------------------------------------------
void __fastcall TForm_caprelvtam2::modificaExecute(TObject *Sender)
{
...
void __fastcall TForm_caprelvtam2::desp_ptvt()
{
     ...
     Frame_datosrelvta1->lkcmb_choferentrega->KeyValue  = qry_entradas->FieldByName("entrega")->AsString;
...
void __fastcall TForm_caprelvtam2::nuevoExecute(TObject *Sender)
{
...
void __fastcall TForm_caprelvtam2::primeroExecute(TObject *Sender)
{
  qry_entradas->First();
  //String sql_z;
...
void __fastcall TForm_caprelvtam2::anteriorExecute(TObject *Sender)
{
  qry_entradas->Prior();
  //String sql_z;
...
void __fastcall TForm_caprelvtam2::siguienteExecute(TObject *Sender)
{
  qry_entradas->Next();
  //String sql_z;
...
//---------------------------------------------------------------------------
void __fastcall TForm_caprelvtam2::ultimoExecute(TObject *Sender)
{
  qry_entradas->Last();
  //String sql_z;
...
void __fastcall TForm_caprelvtam2::alta_ptvt(int modo_z)
{
   int idrelvta_z= 0, idtipoentrega_z=0, idciudad_z=0, idnombre_z=0,
       ...
       numero_z=0, nument_z=0, idtipopago_z=0;
  ...
  qry_sigent->Open();
  if(!qry_sigent->IsEmpty()) nument_z = qry_sigent->FieldByName("sigent")->AsInteger;
  nument_z += 1;
  //date_fecha->Date    = Now();
  ...
  Frame_datosrelvta2->qry_comprometidos->Tag = NO_REFRESH;
  ...
  if( Dlg_analipol->ShowModal() == mrOk) {
    ...
    try {
      ...
      if (modo_z == NUEVO) {
        ...
        qry_entradas->FieldByName("folio")->AsInteger = Frame_datosrelvta2->edt_nument->Value;
      } else {
        idrelvta_z = qry_entradas->FieldByName("idrelvtas")->AsInteger;
        qry_entradas->Edit();
      }
...
void __fastcall TForm_caprelvtam2::modif_ptvt()
{
}
//---------------------------------------------------------------------------

void __fastcall TForm_caprelvtam2::botones_onoff(int modo_z)
{
}
//---------------------------------------------------------------------------

void __fastcall TForm_caprelvtam2::mueve_hacia(String sql_z)
{
...
void __fastcall TForm_caprelvtam2::busca_ptvt(String ptvt_z)
{
...
void __fastcall TForm_caprelvtam2::FormCreate(TObject *Sender)
{
  TIniFile *ini;
  ...
  cia_z = dm->cia_z;
  if(permis_z == "N") {
      Application->MessageBox("Lo siento, no puedes Deshacer Afectaciones", "Permisión Incorrecta", MB_ICONEXCLAMATION);
      Close();
  }
  ...
  busca_polrelvta();
  Frame_datosrelvta1->qry_comprometidos->Tag = SI_REFRESH;
  qry_entradas->Tag = SI_REFRESH;
}
//---------------------------------------------------------------------------

void __fastcall TForm_caprelvtam2::asigna_tipo(int tipo_z)
{
  int idpermis_z = 0;
  entrapor_z = tipo_z;
  switch (entrapor_z) {
...
void __fastcall TForm_caprelvtam2::FormClose(TObject *Sender,
      TCloseAction &Action)
{
  TIniFile *ini;
  String  ubica_z, config_z;
...
void __fastcall TForm_caprelvtam2::edicion_factible(int modo_z)
{
}
//---------------------------------------------------------------------------
...
bool __fastcall TForm_caprelvtam2::afecta_kdx()
{
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_caprelvtam2::exportdbgridExecute(TObject *Sender)
{
...
void __fastcall TForm_caprelvtam2::impritextExecute(TObject *Sender)
{
  ...
  for(qry_impripol->First(); !qry_impripol->Eof; qry_impripol->Next()) {
    ...
    zona_z   = qry_impripol->FieldByName("zonaptvta")->AsString;
    ...
    if (antremate_z != remate_z) {
      if(antremate_z != "-1") {
        reporte_subtot(subtptovta_z, "Total de " + busca_dato("PROMOTOR", antpvta_z), antremate_z);
  ...
  reporte_subtot(subtptovta_z, "Total de " + busca_dato("PROMOTOR", pvta_z), antremate_z);
...
void __fastcall TForm_caprelvtam2::report_encab()
{
...
void __fastcall TForm_caprelvtam2::report_subencabremate()
{
  String titremate_z;
...
void __fastcall TForm_caprelvtam2::report_subencabubi()
{
...
void __fastcall TForm_caprelvtam2::report_subencabzon()
{
   String nombre_z="";
...
void __fastcall TForm_caprelvtam2::report_subencabpvta()
{
   String nombre_z="";
...
String __fastcall TForm_caprelvtam2::busca_dato(String tipodato_z, String clave_z)
{
...
void __fastcall TForm_caprelvtam2::reporte_subtot(double importe_z, String tit_z, String tipo_z)
{
  double PIVA_Z = 16, antesiva_z;
  if(tipo_z == "N") {
    antesiva_z = importe_z / ( PIVA_Z / 100 + 1 );
    fprintf(archout, "%s %s\n",
      rjust(tit_z + " Antes de Iva", 99),
...
void __fastcall TForm_caprelvtam2::qry_renentraAfterScroll(TDataSet *DataSet)
{
...
void __fastcall TForm_caprelvtam2::despliega_renentra()
{
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_caprelvtam2::busca_cod(String codigo_z)
{
}
//---------------------------------------------------------------------------


void __fastcall TForm_caprelvtam2::nueva_polExecute(TObject *Sender)
{
...
void __fastcall TForm_caprelvtam2::alta_pol()
{
  String ubicacion_z;
  int idpolrelvta_z=0;
  TDateTime fecha_z;
...
void __fastcall TForm_caprelvtam2::desp_polrelvta()
{
  int idpolrelvta_z;
  idpolrelvta_z = qry_polrelvta->FieldByName("IDPOLRELVTA")->AsInteger;
  qry_entradas->Close();
...
void __fastcall TForm_caprelvtam2::busca_polrelvta()
{
  String ubicacion_z;
  TDateTime fecha_z;
  ubicacion_z = qry_ubica->FieldByName("clave")->AsString;
...
void __fastcall TForm_caprelvtam2::qry_entradasAfterScroll(TDataSet *DataSet)
{
  if(qry_entradas->Tag == SI_REFRESH) desp_ptvt();
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_caprelvtam2::asigna_almExecute(TObject *Sender)
{
  //if( grabar->Tag != NUEVA_RELACION &&  grabar->Tag != MODIF ) {
  //  mensaje_z = "No esta en Modo de Edicion ";
  //  titmsg_z = "Error";
  ...
  //  qry_renentra->Edit();
  //  qry_renentra->FieldByName("salede")->AsString = qry_disponibles->FieldByName("alm")->AsString;
  //  qry_renentra->FieldByName("folio")->AsString = qry_disponibles->FieldByName("folio")->AsInteger;
  //  qry_renentra->Post();
  //}
}
//---------------------------------------------------------------------------

void __fastcall TForm_caprelvtam2::lkcmb_ubicaKeyDown(TObject *Sender,
...
void __fastcall TForm_caprelvtam2::date_fechaKeyDown(TObject *Sender,
      WORD &Key, TShiftState Shift)
{
  if(Key == VK_RETURN && grabar->Tag == EN_ESPERA) busca_polrelvta();
}
...
void __fastcall TForm_caprelvtam2::ren_sigteExecute(TObject *Sender)
{
 qry_renentra->Next();
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_caprelvtam2::date_fechaExit(TObject *Sender)
{
  busca_polrelvta();
}
//---------------------------------------------------------------------------

void __fastcall TForm_caprelvtam2::lkcmb_ubicaExit(TObject *Sender)
{
  busca_polrelvta();
}
//---------------------------------------------------------------------------
