object Form_mantzona: TForm_mantzona
  Left = 249
  Top = 233
  Width = 611
  Height = 343
  HelpContext = 600
  Caption = 'Mantenimiento Zonas de Manto. Vehicular'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 603
    Height = 51
    ButtonHeight = 47
    Caption = 'ToolBar1'
    Images = dm.ImageList1
    TabOrder = 0
    object SpeedButton4: TSpeedButton
      Left = 0
      Top = 2
      Width = 60
      Height = 47
      Action = nuevo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF0000FFFF00}
      Layout = blGlyphTop
    end
    object SpeedButton3: TSpeedButton
      Left = 60
      Top = 2
      Width = 60
      Height = 47
      Action = modifica
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton2: TSpeedButton
      Left = 120
      Top = 2
      Width = 60
      Height = 47
      Action = borra
      Glyph.Data = {
        
        FF00FF00FF00FF00FF0000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton1: TSpeedButton
      Left = 180
      Top = 2
      Width = 60
      Height = 47
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton5: TSpeedButton
      Left = 240
      Top = 2
      Width = 60
      Height = 47
      Action = WindowClose1
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object ToolButton3: TToolButton
      Left = 300
      Top = 2
      Width = 8
      Caption = 'ToolButton3'
      ImageIndex = 5
      Style = tbsSeparator
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 51
    Width = 603
    Height = 258
    ActivePage = tbs_zonas
    Align = alClient
    TabOrder = 1
    object tbs_zonas: TTabSheet
      Caption = 'Zonas'
      OnShow = tbs_zonasShow
      object dbgrd_zonas: TdxDBGrid
        Left = 0
        Top = 0
        Width = 595
        Height = 230
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'ZONA'
        SummaryGroups = <>
        SummarySeparator = ', '
        Align = alClient
        TabOrder = 0
        DataSource = dts_zonas
        Filter.Criteria = {00000000}
        OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoUseBookmarks, edgoUseLocate]
        OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
        object dbgrd_zonasColumn1: TdxDBGridMaskColumn
          Caption = 'Zona'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'ZONA'
        end
        object dbgrd_zonasColumn2: TdxDBGridMaskColumn
          Caption = 'Nombre'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'NOMBRE'
        end
      end
    end
    object tbs_megazonas: TTabSheet
      Caption = 'Megazonas'
      ImageIndex = 1
      OnShow = tbs_megazonasShow
      object GroupBox1: TGroupBox
        Left = 0
        Top = 0
        Width = 289
        Height = 230
        Align = alLeft
        Caption = 'Megazonas'
        TabOrder = 0
        object dbgrd_megazonas: TdxDBGrid
          Left = 2
          Top = 15
          Width = 285
          Height = 213
          Bands = <
            item
            end>
          DefaultLayout = True
          HeaderPanelRowCount = 1
          KeyField = 'IDMEGAZONA'
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alClient
          TabOrder = 0
          DataSource = dts_megazonas
          Filter.Criteria = {00000000}
          OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoUseBookmarks, edgoUseLocate]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
          object dbgrd_megazonasclave: TdxDBGridMaskColumn
            Caption = 'MegaZona'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'CLAVE'
          end
          object dbgrd_megazonasNombre: TdxDBGridMaskColumn
            Caption = 'Nombre'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'DESCRI'
          end
        end
      end
      object grp_megazonasdet: TGroupBox
        Left = 289
        Top = 0
        Width = 306
        Height = 230
        Align = alClient
        Caption = 'Zonas suscritas'
        TabOrder = 1
        object dbgrd_megazonasdet: TdxDBGrid
          Left = 2
          Top = 49
          Width = 302
          Height = 179
          Bands = <
            item
            end>
          DefaultLayout = True
          HeaderPanelRowCount = 1
          KeyField = 'idmegazonadet'
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alClient
          TabOrder = 0
          DataSource = dts_megazonasdet
          Filter.Criteria = {00000000}
          OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoUseBookmarks, edgoUseLocate]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
          object dts_megazonasdetZONA: TdxDBGridMaskColumn
            Caption = 'Zona'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'ZONA'
          end
          object dts_megazonasdetNOMBRE: TdxDBGridMaskColumn
            Caption = 'Nombre'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'NOMBRE'
          end
        end
        object Panel1: TPanel
          Left = 2
          Top = 15
          Width = 302
          Height = 34
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 1
          object btn_nvoren: TBitBtn
            Left = 5
            Top = 3
            Width = 113
            Height = 25
            Action = agrega_megazonadet
            Caption = 'Agregar'
            ModalResult = 1
            TabOrder = 0
            Glyph.Data = {
              
              3000333333333333377733333333333333333333333333333333}
            NumGlyphs = 2
          end
          object BitBtn1: TBitBtn
            Left = 138
            Top = 3
            Width = 127
            Height = 25
            Action = eliminar_megazonasdet
            Caption = 'Eliminar Suscripcion'
            TabOrder = 1
            Glyph.Data = {
              
              FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
          end
        end
      end
    end
  end
  object qry_zonas: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select * from zonas '
      'order by zona')
    UpdateObject = uqry_ptovta
    Left = 312
    Top = 32
  end
  object uqry_ptovta: TUpdateSQL
    ModifySQL.Strings = (
      'update zonas'
      'set'
      '  NOMBRE = :NOMBRE'
      'where'
      '  ZONA = :OLD_ZONA')
    InsertSQL.Strings = (
      'insert into zonas'
      '  (ZONA, NOMBRE)'
      'values'
      '  (:ZONA, :NOMBRE)')
    DeleteSQL.Strings = (
      'delete from zonas'
      'where'
      '  ZONA = :OLD_ZONA and')
    Left = 342
    Top = 32
  end
  object ActionList1: TActionList
    Images = dm.ImageList1
    Left = 280
    Top = 32
    object nuevo: TAction
      Caption = '&Nuevo'
      ImageIndex = 0
      ShortCut = 16462
      OnExecute = nuevoExecute
    end
    object modifica: TAction
      Caption = '&Modifica'
      Hint = 'Modifica'
      ImageIndex = 1
      ShortCut = 16461
      OnExecute = modificaExecute
    end
    object borra: TAction
      Caption = 'Borra'
      Hint = 'Borra'
      ImageIndex = 2
      ShortCut = 16450
      OnExecute = borraExecute
    end
    object grabar: TAction
      Caption = 'Grabar'
      Enabled = False
      ImageIndex = 9
      ShortCut = 16455
      OnExecute = grabarExecute
    end
    object cancelar: TAction
      Caption = 'Cancelar'
      Enabled = False
      ImageIndex = 11
      ShortCut = 16460
      OnExecute = cancelarExecute
    end
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = 'Salir'
      ImageIndex = 3
    end
    object primero: TAction
      Caption = 'primero'
      Hint = 'Selecciona la Primer Zona'
      ImageIndex = 5
      ShortCut = 49232
      OnExecute = primeroExecute
    end
    object anterior: TAction
      Caption = 'Anterior'
      Hint = 'Selecciona la Anterior Zona'
      ImageIndex = 6
      ShortCut = 49217
      OnExecute = anteriorExecute
    end
    object siguiente: TAction
      Caption = 'Siguiente'
      Hint = 'Selecciona la Siguiente Zona'
      ImageIndex = 8
      ShortCut = 49235
      OnExecute = siguienteExecute
    end
    object ultimo: TAction
      Caption = 'Ultimo'
      Hint = 'Selecciona la Ultima Zona'
      ImageIndex = 7
      ShortCut = 49237
      OnExecute = ultimoExecute
    end
    object exportar: TAction
      Caption = 'Exporta'
      ImageIndex = 19
      OnExecute = exportarExecute
    end
    object agrega_megazonadet: TAction
      Caption = 'Agregar'
      ImageIndex = 13
      OnExecute = agrega_megazonadetExecute
    end
    object eliminar_megazonasdet: TAction
      Caption = 'Eliminar Suscripcion'
      ImageIndex = 15
      OnExecute = eliminar_megazonasdetExecute
    end
  end
  object dts_zonas: TDataSource
    AutoEdit = False
    DataSet = qry_zonas
    Left = 248
    Top = 32
  end
  object qry_megazonas: TQuery
    CachedUpdates = True
    AfterScroll = qry_megazonasAfterScroll
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select * from inv_megazonas where cia = :CIA order by clave')
    UpdateObject = UpdateSQL1
    Left = 376
    Top = 32
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object UpdateSQL1: TUpdateSQL
    ModifySQL.Strings = (
      'update inv_megazonas'
      'set'
      '  DESCRI = :DESCRI,'
      '  CIA = :CIA'
      'where'
      '  IDMEGAZONA = :OLD_IDMEGAZONA')
    InsertSQL.Strings = (
      'insert into inv_megazonas'
      '  (IDMEGAZONA, CLAVE, DESCRI, CIA)'
      'values'
      '  (:IDMEGAZONA, :CLAVE, :DESCRI, :CIA)')
    DeleteSQL.Strings = (
      'delete from inv_megazonas'
      'where'
      '  IDMEGAZONA = :OLD_IDMEGAZONA')
    Left = 406
    Top = 32
  end
  object dts_megazonas: TDataSource
    AutoEdit = False
    DataSet = qry_megazonas
    Left = 440
    Top = 32
  end
  object qry_megazonasdet: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select'
      'a.idmegazonadet,'
      'a.idmegazona,'
      'a.zona,'
      'a.cia,'
      'b.nombre'
      'from inv_megazonasdet a join zonas b on a.zona = b.zona'
      
        'where a.idmegazona = :IDMEGAZONA and a.cia = :CIA order by a.zon' +
        'a'
      '')
    UpdateObject = uqry_megazonasdet
    Left = 472
    Top = 32
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDMEGAZONA'
        ParamType = ptUnknown
      end
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object uqry_megazonasdet: TUpdateSQL
    ModifySQL.Strings = (
      'update inv_megazonasdet'
      'set'
      '  IDMEGAZONADET = :IDMEGAZONADET,'
      '  IDMEGAZONA = :IDMEGAZONA,'
      '  ZONA = :ZONA,'
      '  CIA = :CIA,'
      '  NOMBRE = :NOMBRE'
      'where'
      '  IDMEGAZONADET = :OLD_IDMEGAZONADET')
    InsertSQL.Strings = (
      'insert into inv_megazonasdet'
      '  (IDMEGAZONADET, IDMEGAZONA, ZONA, CIA)'
      'values'
      '  (:IDMEGAZONADET, :IDMEGAZONA, :ZONA, :CIA)')
    DeleteSQL.Strings = (
      'delete from inv_megazonasdet'
      'where'
      '  IDMEGAZONADET = :OLD_IDMEGAZONADET')
    Left = 502
    Top = 32
  end
  object dts_megazonasdet: TDataSource
    AutoEdit = False
    DataSet = qry_megazonasdet
    Left = 536
    Top = 32
  end
end
