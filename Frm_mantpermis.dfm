object Form_mantpermis: TForm_mantpermis
  Left = 311
  Top = 219
  Width = 696
  Height = 480
  Caption = 'Manto. de Permisiones'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 54
    Width = 680
    Height = 36
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 0
    object Label1: TLabel
      Left = 4
      Top = 16
      Width = 42
      Height = 13
      Caption = 'Proyecto'
    end
    object lkcmb_codigo: TDBLookupComboBox
      Left = 54
      Top = 11
      Width = 379
      Height = 21
      DropDownWidth = 300
      KeyField = 'IDPROYECTO'
      ListField = 'DESCRI'
      ListSource = dts_proyectos
      TabOrder = 0
      OnKeyDown = lkcmb_codigoKeyDown
    end
    object DBEdit1: TDBEdit
      Left = 435
      Top = 11
      Width = 84
      Height = 21
      DataField = 'IDPROYECTO'
      DataSource = dts_proyectos
      TabOrder = 1
    end
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 680
    Height = 54
    ButtonHeight = 49
    ButtonWidth = 24
    Caption = 'ToolBar1'
    Images = dm.ImageList1
    TabOrder = 1
    object SpeedButton9: TSpeedButton
      Left = 0
      Top = 2
      Width = 49
      Height = 49
      Action = nuevo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF0000FFFF00}
      Layout = blGlyphTop
    end
    object SpeedButton8: TSpeedButton
      Left = 49
      Top = 2
      Width = 49
      Height = 49
      Action = modifica
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton7: TSpeedButton
      Left = 98
      Top = 2
      Width = 49
      Height = 49
      Action = borra
      Glyph.Data = {
        
        FF00FF00FF00FF00FF0000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton6: TSpeedButton
      Left = 147
      Top = 2
      Width = 49
      Height = 49
      Action = grabar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton5: TSpeedButton
      Left = 196
      Top = 2
      Width = 49
      Height = 49
      Action = cancelar
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton4: TSpeedButton
      Left = 245
      Top = 2
      Width = 49
      Height = 49
      Action = WindowClose1
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object ToolButton3: TToolButton
      Left = 294
      Top = 2
      Width = 8
      Caption = 'ToolButton3'
      ImageIndex = 5
      Style = tbsSeparator
    end
    object SpeedButton3: TSpeedButton
      Left = 302
      Top = 2
      Width = 49
      Height = 49
      Action = primero
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton10: TSpeedButton
      Left = 351
      Top = 2
      Width = 49
      Height = 49
      Action = anterior
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton2: TSpeedButton
      Left = 400
      Top = 2
      Width = 49
      Height = 49
      Action = siguiente
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton1: TSpeedButton
      Left = 449
      Top = 2
      Width = 49
      Height = 49
      Action = ultimo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF000000000000000000}
      Layout = blGlyphTop
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 90
    Width = 680
    Height = 351
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 2
    object dbgrd_cuotacel: TdxDBGrid
      Left = 0
      Top = 0
      Width = 680
      Height = 231
      Bands = <
        item
        end>
      DefaultLayout = True
      HeaderPanelRowCount = 1
      KeyField = 'IDPERMIPROYECTO'
      SummaryGroups = <>
      SummarySeparator = ', '
      Align = alClient
      PopupMenu = PopupMenu1
      TabOrder = 0
      OnKeyDown = dbgrd_cuotacelKeyDown
      DataSource = dts_facpermiproyectos
      Filter.Criteria = {00000000}
      OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoSyncSelection, edgoUseBookmarks]
      OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
      OnCustomDrawCell = dbgrd_cuotacelCustomDrawCell
      object dbgrd_cuotacelColumn2: TdxDBGridMaskColumn
        Caption = 'Descripción'
        BandIndex = 0
        RowIndex = 0
        FieldName = 'DESCRIPCION'
      end
      object dbgrd_cuotacelColumn1: TdxDBGridMaskColumn
        Caption = 'Codigo'
        BandIndex = 0
        RowIndex = 0
        FieldName = 'IDPERMIS'
      end
    end
    object dbgrd_permisiones: TDBGrid
      Left = 0
      Top = 231
      Width = 680
      Height = 120
      Align = alBottom
      DataSource = dts_permisiones
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
    end
  end
  object ActionList1: TActionList
    Images = dm.ImageList1
    Left = 232
    Top = 128
    object nuevo: TAction
      Caption = '&Nuevo'
      Hint = 'Nuevo'
      ImageIndex = 0
      ShortCut = 16462
      OnExecute = nuevoExecute
    end
    object modifica: TAction
      Caption = '&Modifica'
      Hint = 'Modifica'
      ImageIndex = 1
      ShortCut = 16461
      OnExecute = modificaExecute
    end
    object borra: TAction
      Caption = 'Borra'
      Hint = 'Borra'
      ImageIndex = 2
      ShortCut = 16450
      OnExecute = borraExecute
    end
    object grabar: TAction
      Caption = 'Grabar'
      Enabled = False
      ImageIndex = 9
      ShortCut = 16455
    end
    object cancelar: TAction
      Caption = 'Cancelar'
      Enabled = False
      ImageIndex = 11
      ShortCut = 16460
      OnExecute = cancelarExecute
    end
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = '&Salir'
      ImageIndex = 3
    end
    object primero: TAction
      Caption = 'Primero'
      Hint = 'Primero'
      ImageIndex = 5
      ShortCut = 49232
      OnExecute = primeroExecute
    end
    object anterior: TAction
      Caption = 'Anterior'
      Hint = ' Anterior'
      ImageIndex = 6
      ShortCut = 49217
      OnExecute = anteriorExecute
    end
    object siguiente: TAction
      Caption = 'Siguiente'
      Hint = 'Siguiente'
      ImageIndex = 8
      ShortCut = 49235
      OnExecute = siguienteExecute
    end
    object ultimo: TAction
      Caption = 'Ultimo'
      Hint = 'Ultimo'
      ImageIndex = 7
      ShortCut = 49237
      OnExecute = ultimoExecute
    end
    object exportar: TAction
      Caption = 'Exportar a...'
      ImageIndex = 19
      OnExecute = exportarExecute
    end
  end
  object uqry_proyectos: TUpdateSQL
    ModifySQL.Strings = (
      'update fac_proyectos'
      'set'
      '  IDPROYECTO = :IDPROYECTO,'
      '  DESCRI = :DESCRI'
      'where'
      '  IDPROYECTO = :OLD_IDPROYECTO')
    InsertSQL.Strings = (
      'insert into fac_proyectos'
      '  (IDPROYECTO, DESCRI)'
      'values'
      '  (:IDPROYECTO, :DESCRI)')
    DeleteSQL.Strings = (
      'delete from fac_proyectos'
      'where'
      '  IDPROYECTO = :OLD_IDPROYECTO')
    Left = 195
    Top = 160
  end
  object qry_proyectos: TQuery
    CachedUpdates = True
    AfterScroll = qry_proyectosAfterScroll
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select idproyecto, descri from fac_proyectos')
    UpdateObject = uqry_proyectos
    Left = 195
    Top = 128
  end
  object dts_proyectos: TDataSource
    DataSet = qry_proyectos
    Left = 194
    Top = 192
  end
  object qry_permisiones: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select a.idpermis, a.descripcion, a.clave from'
      'car_permisiones a'
      'order by idpermis'
      ' '
      ' ')
    UpdateObject = uqry_permisiones
    Left = 264
    Top = 128
  end
  object uqry_permisiones: TUpdateSQL
    ModifySQL.Strings = (
      'update car_permisiones'
      'set'
      '  DESCRIPCION = :DESCRIPCION, CLAVE = :CLAVE'
      'where'
      '  IDPERMIS = :OLD_IDPERMIS')
    InsertSQL.Strings = (
      'insert into car_permisiones'
      '  (IDPERMIS, DESCRIPCION, clave)'
      'values'
      '  (:IDPERMIS, :DESCRIPCION, :CLAVE)')
    DeleteSQL.Strings = (
      'delete from car_permisiones'
      'where'
      '  IDPERMIS = :OLD_IDPERMIS')
    Left = 264
    Top = 160
  end
  object dts_permisiones: TDataSource
    AutoEdit = False
    DataSet = qry_permisiones
    Left = 263
    Top = 192
  end
  object qry_facpermiproyectos: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      
        'select a.idpermiproyecto, a.idproyecto, a.idpermis, b.descripcio' +
        'n from'
      
        'fac_permiproyectos a join car_permisiones b on a.idpermis = b.id' +
        'permis'
      'where a.idproyecto = :IDPROYECTO'
      'order by descripcion'
      ' ')
    UpdateObject = uqry_facpermiproyectos
    Left = 296
    Top = 130
    ParamData = <
      item
        DataType = ftInteger
        Name = 'IDPROYECTO'
        ParamType = ptUnknown
      end>
  end
  object uqry_facpermiproyectos: TUpdateSQL
    ModifySQL.Strings = (
      'update fac_permiproyectos'
      'set'
      '  IDPERMIPROYECTO = :IDPERMIPROYECTO,'
      '  IDPROYECTO = :IDPROYECTO,'
      '  IDPERMIS = :IDPERMIS'
      'where'
      '  IDPERMIPROYECTO = :OLD_IDPERMIPROYECTO')
    InsertSQL.Strings = (
      'insert into fac_permiproyectos'
      '  (IDPERMIPROYECTO, IDPROYECTO, IDPERMIS)'
      'values'
      '  (:IDPERMIPROYECTO, :IDPROYECTO, :IDPERMIS)')
    DeleteSQL.Strings = (
      'delete from fac_permiproyectos'
      'where'
      '  IDPERMIPROYECTO = :OLD_IDPERMIPROYECTO')
    Left = 296
    Top = 160
  end
  object dts_facpermiproyectos: TDataSource
    DataSet = qry_facpermiproyectos
    Left = 295
    Top = 192
  end
  object PopupMenu1: TPopupMenu
    Left = 80
    Top = 194
    object Exportara1: TMenuItem
      Action = exportar
    end
  end
end
