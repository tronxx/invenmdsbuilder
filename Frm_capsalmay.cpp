...
#pragma link "Frme_datoscapsmay"
#pragma link "Frme_datosrensmay"
#pragma resource "*.dfm"
#include "situaciones.h"
TForm_capsalmay *Form_capsalmay;
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
__fastcall TForm_capsalmay::TForm_capsalmay(TComponent* Owner, String tipo_sabana, bool estado)
    : TForm(Owner)
{
  ...
  Frame_datosrensmay1->inicializa(sTipo);
  Frame_datosrensmay1->dbedt_descriinven->DataSource = dts_renentra;
  qsPuntoVta->Close();
  ...
  alm_z    =  ini->ReadString( "Form_capsalmay_"+ sTipo, "ALMACEN", alm_z );
  numero_z =  ini->ReadInteger( "Form_capsalmay_"+ sTipo, "NUMERO", 0);
  fechaabierta_z  = ini->ReadString( "Form_capsalmay_"+ sTipo, "FECHA_ABIERTA", "N");
  delete ini;
  Frame_datoscapsmay1->dbedt_importe->DataSource = dts_entradas;
  Frame_datoscapsmay1->dbedt_iva->DataSource = dts_entradas;
  Frame_datoscapsmay1->dbedt_total->DataSource = dts_entradas;
  Frame_datoscapsmay1->lkcmb_almacen->KeyValue = alm_z;
  Frame_datoscapsmay1->inicializa(sTipo);
  Frame_datoscapsmay1->lkcmb_almacen->Enabled = false;
  Frame_datoscapsmay1->lkcmb_mayoris->Enabled = false;
  Frame_datoscapsmay1->cmb_siono->Enabled = false;
  Frame_datoscapsmay1->lkcmb_facorem->Enabled = false;
  Frame_datoscapsmay1->date_fecha->Enabled = false;
  SigteSabana(ULTIMO);
}
//---------------------------------------------------------------------------


bool __fastcall TForm_capsalmay::BuscarEntrada(int iNumero)
{
    bool bResult = false;
    String alm_z, mayoris_z;
    int siono_z;
    alm_z = Frame_datoscapsmay1->qry_Almacen->FieldByName("clave")->AsString;
    qsEntradas->Close();
    ...
    if (!qsEntradas->IsEmpty() ) {
       ...
       //ListaRenglones;
       Frame_datoscapsmay1->edt_numero->Value= iNumero;
       Frame_datoscapsmay1->date_fecha->Date = qsEntradas->FieldByName("Fecha")->AsDateTime;
       mayoris_z = qsEntradas->FieldByName("prove")->AsString;
       Frame_datoscapsmay1->lkcmb_mayoris->KeyValue = mayoris_z;
       if(qsEntradas->FieldByName("coniva")->AsString == "S")
         siono_z = 0;
       else
         siono_z = 1;
       Frame_datoscapsmay1->cmb_siono->ItemIndex = siono_z;
       Frame_datoscapsmay1->lkcmb_facorem->KeyValue  = qsEntradas->FieldByName("letras")->AsInteger;
       Frame_datoscapsmay1->edt_numfac->Value = qsEntradas->FieldByName("planp")->AsInteger;
       bResult = true;
    }
    ListarRenglon();
...
void __fastcall TForm_capsalmay::btn_salirClick(TObject *Sender)
{
    Close();
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::ListarRenglon()
{
...
void __fastcall TForm_capsalmay::qry_renentraAfterScroll(TDataSet *DataSet)
{
  ...
  Frame_datosrensmay1->edt_codigo->Text = qry_renentra->FieldByName("codinv")->AsString;
  Frame_datosrensmay1->lst_series->Items->Clear();
  Frame_datosrensmay1->lst_series->Items->Add(qry_renentra->FieldByName("serie")->AsString);
  ...
  //Frame_datosrensmay1->edt_descri->Text = descripcion_z;
  Frame_datosrensmay1->edt_folios->Text = folios_z;
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::BuscarVendedor(String sVendedor)
{
}
//---------------------------------------------------------------------------
void __fastcall TForm_capsalmay::BuscarPoblacion(int iPoblac)
{
}
//---------------------------------------------------------------------------
void __fastcall TForm_capsalmay::BuscarPuntoVenta(String sClave)
{
}
//---------------------------------------------------------------------------
void __fastcall TForm_capsalmay::BuscarConcepto(int iCliente)
{
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::FormClose(TObject *Sender,
      TCloseAction &Action)
{
  String alm_z;
  int numero_z;
  if(qsEntradas->State != dsInactive) {
    ...
    config_z = dm->busca_inifile();
    ini = new TIniFile(config_z);
    ini->WriteString( "Form_casalmay_"+ sTipo, "ALMACEN", alm_z );
    ini->WriteInteger( "Form_capsalmay_"+ sTipo, "NUMERO", numero_z);
    delete ini;
  }
  Action = caFree;
}
...
void __fastcall TForm_capsalmay::SigteSabana(int tipo_mov)
{
  int nument_z=0;
  String alm_z;
  alm_z = Frame_datoscapsmay1->qry_Almacen->FieldByName("clave")->AsString;
  nument_z = Frame_datoscapsmay1->edt_numero->Value;
  nument_z = dm->sigte_entrada(alm_z, sTipo, nument_z, tipo_mov);
  Frame_datoscapsmay1->edt_numero->Value = nument_z;
  BuscarEntrada(nument_z);
}
//---------------------------------------------------------------------------


void __fastcall TForm_capsalmay::primeroExecute(TObject *Sender)
{
   SigteSabana(PRIMERO);
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::anteriorExecute(TObject *Sender)
{
   SigteSabana(ANTER);
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::siguienteExecute(TObject *Sender)
{
   SigteSabana(SIGTE);
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::ultimoExecute(TObject *Sender)
{
   SigteSabana(ULTIMO);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm_capsalmay::nuevoExecute(TObject *Sender)
{
   int ancho_z, alto_z, numero_z=0, idfacorem_z=0, numfac_z=0;
   String alm_z, strfecha_z, strfechoy_z, strfecatras_z, mayoris_z,
   siono_z;
   double fecha_z;
   bool todook_z = true;
   ...
   Frame_datoscapsmay = new TFrame_datoscapsmay (Dlg_analipol);
   ancho_z = Frame_datoscapsmay->Width + 20;
   alto_z = Frame_datoscapsmay->Height + Dlg_analipol->Panel2->Height + 80;
   Frame_datoscapsmay->Parent = Dlg_analipol->Panel1;
   Dlg_analipol->Width = ancho_z;
   Dlg_analipol->Height = alto_z;
   Dlg_analipol->Caption = "Captura de Datos";
   Frame_datoscapsmay->Align = alClient;
   Frame_datoscapsmay->inicializa(sTipo);
   Frame_datoscapsmay->accion_z = NUEVO;
   Dlg_analipol->Position = poScreenCenter;
   alm_z    = Frame_datoscapsmay1->qry_Almacen->FieldByName("clave")->AsString;

   Frame_datoscapsmay->lkcmb_almacen->KeyValue = alm_z;
   Frame_datoscapsmay->lkcmb_mayoris->KeyValue = Frame_datoscapsmay1->qry_mayorista->FieldByName("codigo")->AsString;
   Frame_datoscapsmay->cmb_siono->ItemIndex = Frame_datoscapsmay1->cmb_siono->ItemIndex;
   Frame_datoscapsmay->lkcmb_facorem->KeyValue = Frame_datoscapsmay1->lkcmb_facorem->KeyValue;
   Frame_datoscapsmay->edt_numfac->Value= Frame_datoscapsmay1->edt_numfac->Value;
   Frame_datoscapsmay->nuevo();

   if( Dlg_analipol->ShowModal() == mrOk) {
     alm_z    = Frame_datoscapsmay->qry_Almacen->FieldByName("clave")->AsString;
     fecha_z  = Frame_datoscapsmay->date_fecha->Date;
     numero_z = Frame_datoscapsmay->edt_numero->Value;
     mayoris_z = Frame_datoscapsmay->qry_mayorista->FieldByName("codigo")->AsString;
     siono_z   = Frame_datoscapsmay->cmb_siono->Text;
     idfacorem_z = Frame_datoscapsmay->qry_tipofacmay->FieldByName("idtipofacturma")->AsInteger;
     numfac_z  =   Frame_datoscapsmay->edt_numfac->Value;
     ...
     if(todook_z) {
       alta_ptvt(alm_z, fecha_z, numero_z, mayoris_z,
        siono_z, idfacorem_z, numfac_z);
     }
   }
   delete Frame_datoscapsmay;
   delete Dlg_analipol;
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::botones_onoff(int modo_z)
{
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::borraExecute(TObject *Sender)
{
  bool si_borrar = true;
  String alm_z;
  int nument_z = 0;
...
void __fastcall TForm_capsalmay::modif_ptvt()
{
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::
alta_ptvt(
    ...
    )
{
   ...
   if(!qsEntradas->IsEmpty()) return;
   Frame_datoscapsmay1->lkcmb_almacen->KeyValue = alm_z;
   nombre_z = Frame_datoscapsmay1->qry_mayorista->FieldByName("nombre")->AsString;
   idnombre_z = dm->busca_iddato(nombre_z, OLDCONCEPTO);
...
// fechaprp,ctofincomp,usuario,cia,
   ...
   try {
     ...
     qsEntradas->FieldByName("prove")->AsString = mayoris_z;
     qsEntradas->FieldByName("perenvrec")->AsInteger = idnombre_z;
     qsEntradas->FieldByName("status")->AsString = "A";
     qsEntradas->FieldByName("coniva")->AsString = siono_z;
     ...
     qsEntradas->FieldByName("letras")->AsInteger = idfacorem_z;
     qsEntradas->FieldByName("fletes")->AsInteger = 0;
     qsEntradas->FieldByName("desxap")->AsInteger = 0;
     qsEntradas->FieldByName("planp")->AsInteger = numfac_z;
...
void __fastcall TForm_capsalmay::agrerenExecute(TObject *Sender)
{
   ...
   Frame_datosrensmay = new TFrame_datosrensmay (Dlg_analipol);
   ancho_z = Frame_datosrensmay->Width + 20;
   alto_z = Frame_datosrensmay->Height + Dlg_analipol->Panel2->Height + 80;
   Frame_datosrensmay->Parent = Dlg_analipol->Panel1;
   alm_z = Frame_datoscapsmay1->qry_Almacen->FieldByName("clave")->AsString;
   fecha_z = qsEntradas->FieldByName("fecha")->AsDateTime;
   Frame_datosrensmay->fecha_z = fecha_z;
   Frame_datosrensmay->alm_z = alm_z;
   Dlg_analipol->Width = ancho_z;
   Dlg_analipol->Height = alto_z;
   Dlg_analipol->Caption = "Captura de Ventas";
   Dlg_analipol->Position = poScreenCenter;
   if( Dlg_analipol->ShowModal() == mrOk) {
      cantmueve_z = Frame_datosrensmay1->edt_canti->Value;
      if(cantmueve_z < 1) {
        Application->MessageBox(("No pueden ser "+ IntToStr((int)Frame_datosrensmay1->edt_canti->Value) + " Articulos").c_str(), "Incorrecto Número de Artículos", MB_ICONEXCLAMATION);
        return;
      }
      tipo_z = Frame_datosrensmay->qry_inven->FieldByName("tipo")->AsString;
      descri_z = Frame_datosrensmay->qry_inven->FieldByName("descri")->AsString;
      crdocon_z = 1;

      if(tipo_z == "ALF" && (Frame_datosrensmay->lst_series->Items->Count != Frame_datosrensmay->edt_canti->Value))  {
        Application->MessageBox(("Debe proporcionar "+ IntToStr((int)Frame_datosrensmay->edt_canti->Value) + " Series").c_str(), "Incorrecto Número de Series", MB_ICONEXCLAMATION);
        return;
      }
      if(!Frame_datosrensmay->qry_folsel->State == dsInactive)
        Frame_datosrensmay->qry_folsel->First();
      vendedor_z = "";
      ...
      alm_z = Frame_datoscapsmay1->qry_Almacen->FieldByName("clave")->AsString;
      ptovta_z = "";
      codigo_z = Frame_datosrensmay->qry_inven->FieldByName("codigo")->AsString;
      piva_z   = Frame_datosrensmay->qry_inven->FieldByName("piva")->AsFloat;
      nombre_z = ""; //Frame_datosrensmay->edt_nombre->Text.Trim();
      idpers_z = qsEntradas->FieldByName("perenvrec")->AsInteger;
      idciudad_z = 0;
      prvta_z = 0;
     ...
     try {
       ...
       dm->manvehi->StartTransaction();
       for(ii_z = 0; ii_z < Frame_datosrensmay->edt_canti->Value; ii_z++) {
         if(tipo_z == "ALF" || dm->foliosalf_z ) {
           serie_z = Frame_datosrensmay->qry_folsel->FieldByName("serie")->AsString;
           //idserie_z = dm->busca_iddato(serie_z, SERIE);
           folio_z = Frame_datosrensmay->qry_folsel->FieldByName("folio")->AsInteger;
         } else {
           idserie_z = 0;
           serie_z = "";
         }
         ...
         //nombre_z = Frame_datosrensmay->edt_nombre->Text;
         status_z = ""; //Frame_datosrensmay->cmb_status->Text;
         ...
         qsEntradas->FieldByName("total")->AsFloat         += costo_z * ((piva_z / 100) + 1);
         qsEntradas->Post();
         Frame_datosrensmay->qry_folsel->Next();
       }
       qsEntradas->ApplyUpdates();
       qry_renentra->ApplyUpdates();
       dm->manvehi->Commit();
   ...
   delete Frame_datosrensmay;
   delete Dlg_analipol;
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::edicion_factible(int modo_z)
{
  ...
  Frame_datosrensmay1->edt_codigo->ReadOnly = modo_readonly;
  Frame_datosrensmay1->edt_canti->ReadOnly = modo_readonly;
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::CierraRenExecute(TObject *Sender)
{
  bool cierre_ren_z;
...
void __fastcall TForm_capsalmay::cancela_renglonExecute(TObject *Sender)
{
...
void __fastcall TForm_capsalmay::dbgrdRengCustomDrawCell(TObject *Sender,
...
void __fastcall TForm_capsalmay::exporta_dbgrdExecute(TObject *Sender)
{
...
void __fastcall TForm_capsalmay::imprimeExecute(TObject *Sender)
{
  double piva_z=0, importe_z=0, iva_z=0, total_z=0, costo_z=0,
     totfac_z=0, totcos_z=0, mub_z=-100;
  int ii_z=1, numfac_z, canti_z=0;
  String arch_z = Name+".txt", serie_z="", vnd_z="", codigo_z="",
  descritipofac_z, antcod_z="";
  if(qsEntradas->FieldByName("status")->AsString != "C") {
  ...
  numfac_z = qsEntradas->FieldByName("planp")->AsInteger;
  descritipofac_z = Frame_datoscapsmay1->qry_tipofacmay->FieldByName("descri")->AsString;
...
//    zona_z = qry_renpogas->FieldByName("zona")->AsInteger;
  ...
  for(qry_renentra->First(); !qry_renentra->Eof; qry_renentra->Next()) {
    checa_salto(3);
    codigo_z = qry_renentra->FieldByName("codinv")->AsString;
    if(antcod_z == "-1") antcod_z = codigo_z;
    if(codigo_z != antcod_z) {
      subtot_x_codigo(antcod_z, canti_z, totcos_z, totfac_z);
      antcod_z = codigo_z;
      canti_z  = 0;
      totcos_z = 0;
    }

    //Frame_datosrensmay1->lkcmb_poblac->KeyValue = qry_renentra->FieldByName("Poblac")->AsInteger;
    if( sTipo != "Q") {
        costo_z = qry_renentra->FieldByName("costou")->AsFloat;
    } else {
        costo_z = qry_renentra->FieldByName("prvta")->AsFloat;
    }
    canti_z += qry_renentra->FieldByName("unids")->AsInteger;
    totcos_z += costo_z;
  ...
  fprintf(archout, "%s\n",
    rjust(
      ...
      "   Total:" + FormatFloat("#,###,##0.00", total_z), 84
    )
  );
  if(totfac_z) mub_z = (1 - (importe_z / totfac_z)) * 100;
  fprintf(archout, "Total Precio Facturacion Segun ");
  fprintf(archout, "%s", ljust(descritipofac_z,10));
  fprintf(archout, "%s:", rjust(FormatFloat("0", numfac_z),6));
  fprintf(archout, "%s ", rjust(FormatFloat("###,##0.00", totfac_z), 12));
  fprintf(archout, "MUB:%s", rjust(FormatFloat("0.00", mub_z), 12));
  salta_linea();
  fclose(archout);
...
void __fastcall TForm_capsalmay::salta_linea()
{
  fprintf(archout, "\n");
  lineas_z++;
}

void __fastcall TForm_capsalmay::subtot_x_codigo(String codigo_z, int canti_z, double totcos_z, double &totalfactur_z)
{
  int unids_z;
  double mub_z=-100, totfac_z=0;
  String sql_z, descritipofac_z;
  descritipofac_z = Frame_datoscapsmay1->qry_tipofacmay->FieldByName("descri")->AsString;
  sql_z = "select sum(unids) as unids, sum(importe) as importe";
  sql_z += " from renentra where tipo = :TIPO and alm = :ALM and numero = :NUMERO and cia = :CIA";
  sql_z += " and codinv = :CODIGO";
  ejecuta_sql(sql_z, codigo_z);
  unids_z   = dm->qry_maxidvnd->FieldByName("unids")->AsInteger;
  //totcos_z  = dm->qry_maxidvnd->FieldByName("importe")->AsFloat;
  dm->qry_maxidvnd->Close();
  sql_z = "select sum(unids) as unids, sum(b.importe - b.descu) as importe";
  sql_z += " from facturma a join renfacma b on a.idfactur = b.idfactur ";
  sql_z += " where a.num = :NUMFAC and a.tipofac = :TIPOFAC and a.cia = :CIA";
  sql_z += " and codigo = :CODIGO";
  ejecuta_sql(sql_z, codigo_z);
  totfac_z = dm->qry_maxidvnd->FieldByName("importe")->AsFloat;
  dm->qry_maxidvnd->Close();
  if(unids_z != canti_z && canti_z != 0) {
    totfac_z = totfac_z / unids_z * canti_z;
  }
  if(totfac_z !=  0) mub_z = (1-(totcos_z / totfac_z)) * 100;
  fprintf(archout, "%sTotal de Este Articulo:%s", ljust("", 6), ljust("", 16));
  fprintf(archout, "%s ", rjust(IntToStr(canti_z), 5));
  fprintf(archout, "%s ", rjust(FormatFloat("###,##0.00", totcos_z), 12));
  fprintf(archout, " MUB: %s ", ljust(FormatFloat("0.00", mub_z), 5));
  salta_linea();
  fprintf(archout, "%sPrecio de Venta Segun %s", ljust("", 6), ljust(descritipofac_z, 23) );
  fprintf(archout, "%s ", rjust(FormatFloat("###,##0.00", totfac_z), 12));
  totalfactur_z += totfac_z;
  salta_linea();
  salta_linea();
}
//---------------------------------------------------------------------------

void __fastcall TForm_capsalmay::ejecuta_sql(String sql_z, String codigo_z)
{
   String nombre_z="", nombreparam_z, alm_z;
   int nument_z, numfac_z, tipofac_z;
   alm_z = qsEntradas->FieldByName("alm")->AsString;
   nument_z = qsEntradas->FieldByName("numero")->AsInteger;
   numfac_z = qsEntradas->FieldByName("planp")->AsInteger;
   tipofac_z = qsEntradas->FieldByName("letras")->AsInteger;
   dm->qry_maxidvnd->Close();
   ...
   for(int jj_z=0; jj_z < dm->qry_maxidvnd->Params->Count; jj_z++) {
      ...
      if( nombreparam_z == "TIPO") {
        dm->qry_maxidvnd->Params->Items[jj_z]->AsString = sTipo;
      }
      if( nombreparam_z == "CODIGO") {
        dm->qry_maxidvnd->Params->Items[jj_z]->AsString = codigo_z;
      }
      if( nombreparam_z == "ALM") {
        dm->qry_maxidvnd->Params->Items[jj_z]->AsString = alm_z;
      }
      if( nombreparam_z == "NUMERO") {
        dm->qry_maxidvnd->Params->Items[jj_z]->AsInteger = nument_z;
      }
      if( nombreparam_z == "NUMFAC") {
        dm->qry_maxidvnd->Params->Items[jj_z]->AsInteger = numfac_z;
      }
      if( nombreparam_z == "TIPOFAC") {
        dm->qry_maxidvnd->Params->Items[jj_z]->AsInteger = tipofac_z;
      }
...
bool __fastcall TForm_capsalmay::checa_salto( int maxlin_z)
{
   bool salto_z= false;
   if(lineas_z > ( LIN_X_PAG - maxlin_z)) {
...
void __fastcall TForm_capsalmay::report_encab()
{
  ...
  fprintf(archout, "Numero:%6d Almacen:%s %s Fecha:%s\n",
    ...
    Frame_datoscapsmay1->qry_Almacen->FieldByName("clave")->AsString,
    Frame_datoscapsmay1->qry_Almacen->FieldByName("nombre")->AsString,
    FormatDateTime("dd-mm-yyyy", qsEntradas->FieldByName("fecha")->AsDateTime)
  );
  fprintf(archout, "Mayorista:%s %s",
    Frame_datoscapsmay1->qry_mayorista->FieldByName("codigo")->AsString,
    Frame_datoscapsmay1->qry_mayorista->FieldByName("nombre")->AsString
  );
  fprintf(archout, " Status:%s", qsEntradas->FieldByName("coniva")->AsString);
  fprintf(archout, " %s %s\n",
    Frame_datoscapsmay1->qry_tipofacmay->FieldByName("descri")->AsString,
    IntToStr(qsEntradas->FieldByName("planp")->AsInteger)
  );
  fprintf(archout, "%s%s",
...
void __fastcall TForm_capsalmay::cancelarExecute(TObject *Sender)
{
...
void __fastcall TForm_capsalmay::capobsExecute(TObject *Sender)
{
...
void __fastcall TForm_capsalmay::deshacer_afectacionExecute(
      TObject *Sender)
{
...
void __fastcall TForm_capsalmay::cambia_pto_vtaExecute(TObject *Sender)
{
  bool cierre_ren_z;
  if (qry_renentra->IsEmpty()) return;
  if(qry_renentra->IsEmpty()) return;
...
void __fastcall TForm_capsalmay::dbgrdRengKeyDown(TObject *Sender,
...
void __fastcall TForm_capsalmay::cierra_entradaExecute(TObject *Sender)
{
  bool si_cerrar = true;
  String alm_z;
  int nument_z = 0;
...
void __fastcall TForm_capsalmay::Frame_datoscapsmay1edt_numeroKeyDown(
      TObject *Sender, WORD &Key, TShiftState Shift)
{
    if (Shift == TShiftState() ) {
       if (Key != VK_RETURN) return;
       if (!BuscarEntrada(Frame_datoscapsmay1->edt_numero->Value) ) {
...
void __fastcall TForm_capsalmay::despliega_menuExecute(TObject *Sender)
{
