...
void __fastcall TForm_capcancel::agrerenExecute(TObject *Sender)
{
   ...
   if( Dlg_analipol->ShowModal() == mrOk) {
      ...
      if(cantmueve_z < 1) {
        Application->MessageBox(("No pueden ser "+ IntToStr((int)Frame_datoscapcancelagrega->edt_canti->Value) + " Articulos").c_str(), "Incorrecto Número de Artículos", MB_ICONEXCLAMATION);
        return;
      }
...
void __fastcall TForm_capcancel::CierraRenExecute(TObject *Sender)
{
  ...
  mensaje_z = "Esta seguro de cerrar este Renglón y afectar Kardex ? \n una vez cerrado No puede dar marcha atrás";
  titmsg_z = "Cerrar Renglón";
...
void __fastcall TForm_capcancel::abrerenExecute(TObject *Sender)
{
  ...
  int ii_z = Application->MessageBox("Esta Seguro de Deshacer la afectación de este Renglón ?", "Deshacer afectación Kardex", MB_OKCANCEL | MB_ICONQUESTION | MB_DEFBUTTON2);
  if (ii_z != IDOK ) return;
  siono_z = qry_renentra->FieldByName("siono")->AsString;
  folio_z = qry_renentra->FieldByName("folsal")->AsInteger;
  codigo_z = qry_renentra->FieldByName("codinv")->AsString;
...
void __fastcall TForm_capcancel::nuevoExecute(TObject *Sender)
{
   ...
   if( Dlg_analipol->ShowModal() == mrOk) {
     ...
     if( strfecha_z < strfecatras_z ) {
       Application->MessageBox("Lo siento, no puede agregar fechas de mas de 90 días de diferencia", "Fecha Incorrecta", MB_ICONEXCLAMATION);
       todook_z = false;
     }
