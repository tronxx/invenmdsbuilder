object Form_mantousr: TForm_mantousr
  Left = 315
  Top = 190
  Width = 699
  Height = 414
  Caption = 'Mantenimiento de Usuarios'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 683
    Height = 54
    ButtonHeight = 49
    Caption = 'ToolBar1'
    Images = dm.ImageList1
    TabOrder = 0
    object SpeedButton6: TSpeedButton
      Left = 0
      Top = 2
      Width = 49
      Height = 49
      Action = nuevo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF0000FFFF00}
      Layout = blGlyphTop
    end
    object SpeedButton5: TSpeedButton
      Left = 49
      Top = 2
      Width = 49
      Height = 49
      Action = modifica
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton4: TSpeedButton
      Left = 98
      Top = 2
      Width = 49
      Height = 49
      Action = borra
      Glyph.Data = {
        
        FF00FF00FF00FF00FF0000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton1: TSpeedButton
      Left = 147
      Top = 2
      Width = 49
      Height = 49
      Action = WindowClose1
      Glyph.Data = {
        
        0000000000000000000000000000FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object ToolButton2: TToolButton
      Left = 196
      Top = 2
      Width = 8
      Caption = 'ToolButton2'
      ImageIndex = 4
      Style = tbsSeparator
    end
    object SpeedButton10: TSpeedButton
      Left = 204
      Top = 2
      Width = 49
      Height = 49
      Action = primero
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton9: TSpeedButton
      Left = 253
      Top = 2
      Width = 49
      Height = 49
      Action = anterior
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton8: TSpeedButton
      Left = 302
      Top = 2
      Width = 49
      Height = 49
      Action = siguiente
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      Layout = blGlyphTop
    end
    object SpeedButton7: TSpeedButton
      Left = 351
      Top = 2
      Width = 49
      Height = 49
      Action = ultimo
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF000000000000000000}
      Layout = blGlyphTop
    end
  end
  object pgctrl_datos: TPageControl
    Left = 0
    Top = 54
    Width = 683
    Height = 321
    ActivePage = tbs_usuarios
    Align = alClient
    TabOrder = 1
    object tbs_usuarios: TTabSheet
      Caption = 'Usuarios'
      OnShow = tbs_usuariosShow
      object dbgrd_usuarios: TdxDBGrid
        Left = 0
        Top = 0
        Width = 675
        Height = 293
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'IDUSUARIO'
        SummaryGroups = <>
        SummarySeparator = ', '
        Align = alClient
        PopupMenu = PopupMenu1
        TabOrder = 0
        OnDblClick = dbgrd_usuariosDblClick
        OnKeyDown = dbgrd_usuariosKeyDown
        DataSource = dts_usuario
        Filter.Criteria = {00000000}
        OptionsBehavior = [edgoAutoSearch, edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
        OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoSyncSelection, edgoUseBookmarks, edgoUseLocate]
        OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
        object dbgrd_usuariosCODIGO: TdxDBGridMaskColumn
          Caption = 'Código'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'IDUSUARIO'
        end
        object dbgrd_usuariosLOGIN: TdxDBGridMaskColumn
          Caption = 'Usuario'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'LOGIN'
        end
        object dbgrd_usuariosCLAVE: TdxDBGridMaskColumn
          Caption = 'Password'
          PasswordChar = '*'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'CLAVE'
        end
        object dbgrd_usuariosMAESTRO: TdxDBGridMaskColumn
          Caption = 'Maestro'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'MAESTRO'
        end
        object dbgrd_usuariosNUMPOL: TdxDBGridMaskColumn
          Caption = 'Póliza'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'NUMPOL'
        end
        object dbgrd_usuariosINICIALES: TdxDBGridMaskColumn
          Caption = 'Iniciales'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'INICIALES'
        end
        object dbgrd_usuariosNOMBRE: TdxDBGridMaskColumn
          Caption = 'Nombre'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'NOMBRE'
        end
        object dbgrd_usuariosSTATUS: TdxDBGridMaskColumn
          Caption = 'Status'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'STATUS'
        end
        object dbgrd_usuariosFECBAJA: TdxDBGridDateColumn
          Caption = 'Fecha Baja'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'fecbaj'
        end
      end
    end
    object tbs_perfiles: TTabSheet
      Caption = 'Perfiles'
      ImageIndex = 1
      OnShow = tbs_usuariosShow
      object dbgrd_perfiles: TdxDBGrid
        Left = 0
        Top = 0
        Width = 675
        Height = 293
        Bands = <
          item
          end>
        DefaultLayout = True
        HeaderPanelRowCount = 1
        KeyField = 'IDPERFIL'
        SummaryGroups = <>
        SummarySeparator = ', '
        Align = alClient
        PopupMenu = PopupMenu1
        TabOrder = 0
        OnDblClick = dbgrd_usuariosDblClick
        OnKeyDown = dbgrd_usuariosKeyDown
        DataSource = dts_perfiles
        Filter.Criteria = {00000000}
        OptionsBehavior = [edgoAutoSearch, edgoAutoSort, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
        OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoSyncSelection, edgoUseBookmarks, edgoUseLocate]
        OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
        object dbgrd_perfilesCLAVE: TdxDBGridMaskColumn
          Caption = 'Clave'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'CLAVE'
        end
        object dbgrd_perfilesDESCRI: TdxDBGridMaskColumn
          Caption = 'Descripción'
          BandIndex = 0
          RowIndex = 0
          FieldName = 'DESCRI'
        end
      end
    end
  end
  object qry_usuario: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      
        'select a.idusuario, a.idnombre, a.login, a.clave, a.maestro, a.n' +
        'umpol,'
      'a.iniciales, a.status, a.fecbaj, b.nombre'
      'from car_usuarios  a join pvenfimo b on a.idnombre = b.idnombre'
      'order by login'
      ' ')
    UpdateObject = uqry_usuario
    Left = 120
    Top = 232
  end
  object uqry_usuario: TUpdateSQL
    ModifySQL.Strings = (
      'update car_usuarios'
      'set'
      '  IDUSUARIO = :IDUSUARIO,'
      '  IDNOMBRE = :IDNOMBRE,'
      '  LOGIN = :LOGIN,'
      '  CLAVE = :CLAVE,'
      '  MAESTRO = :MAESTRO,'
      '  NUMPOL = :NUMPOL,'
      '  INICIALES = :INICIALES'
      'where'
      '  IDUSUARIO = :OLD_IDUSUARIO')
    InsertSQL.Strings = (
      'insert into car_usuarios'
      
        '  (IDUSUARIO, IDNOMBRE, LOGIN, CLAVE, MAESTRO, NUMPOL, INICIALES' +
        ')'
      'values'
      
        '  (:IDUSUARIO, :IDNOMBRE, :LOGIN, :CLAVE, :MAESTRO, :NUMPOL, :IN' +
        'ICIALES)')
    DeleteSQL.Strings = (
      'delete from car_usuarios'
      'where'
      '  IDUSUARIO = :OLD_IDUSUARIO')
    Left = 150
    Top = 232
  end
  object ActionList1: TActionList
    Images = dm.ImageList1
    Left = 152
    Top = 264
    object nuevo: TAction
      Caption = '&Nuevo'
      Hint = 'Nuevo'
      ImageIndex = 0
      ShortCut = 16462
      OnExecute = nuevoExecute
    end
    object modifica: TAction
      Caption = '&Modifica'
      Hint = 'Modifica'
      ImageIndex = 1
      ShortCut = 16461
      OnExecute = modificaExecute
    end
    object borra: TAction
      Caption = 'Borra'
      Hint = 'Borra'
      ImageIndex = 2
      ShortCut = 16450
      OnExecute = borraExecute
    end
    object grabar: TAction
      Caption = 'Grabar'
      Enabled = False
      Hint = 'Grabar'
      ImageIndex = 9
      ShortCut = 16455
      OnExecute = grabarExecute
    end
    object cancelar: TAction
      Caption = 'Cancelar'
      Enabled = False
      Hint = 'Cancela'
      ImageIndex = 11
      ShortCut = 16460
      OnExecute = cancelarExecute
    end
    object WindowClose1: TWindowClose
      Category = 'Window'
      Caption = '&Salir'
      ImageIndex = 3
    end
    object primero: TAction
      Caption = 'Primero'
      Hint = 'Selecciona al Primer Usuario'
      ImageIndex = 5
      ShortCut = 49232
      OnExecute = primeroExecute
    end
    object anterior: TAction
      Caption = 'Anterior'
      Hint = 'Selecciona al Anterior Usuario'
      ImageIndex = 6
      ShortCut = 49217
      OnExecute = anteriorExecute
    end
    object siguiente: TAction
      Caption = 'Siguiente'
      Hint = 'Selecciona al Siguiente Usuario'
      ImageIndex = 8
      ShortCut = 49235
      OnExecute = siguienteExecute
    end
    object ultimo: TAction
      Caption = 'Ultimo'
      Hint = 'Selecciona al Ultimo Usuario'
      ImageIndex = 7
      ShortCut = 49237
      OnExecute = ultimoExecute
    end
    object exportar: TAction
      Caption = 'Exporta a ...'
      ImageIndex = 19
      OnExecute = exportarExecute
    end
  end
  object qry_permisiones: TQuery
    DatabaseName = 'manvehi'
    SQL.Strings = (
      
        'select idpermis, descripcion from car_permisiones order by descr' +
        'ipcion')
    Left = 184
    Top = 232
  end
  object dts_permisiones: TDataSource
    DataSet = qry_permisiones
    Left = 182
    Top = 264
  end
  object qry_permisos: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select permiso, descripcion '
      'from car_permisos'
      'order by permiso')
    UpdateObject = uqry_permisos
    Left = 218
    Top = 232
  end
  object dts_permisos: TDataSource
    DataSet = qry_permisos
    Left = 218
    Top = 264
  end
  object uqry_permisos: TUpdateSQL
    Left = 220
    Top = 299
  end
  object dts_usuario: TDataSource
    DataSet = qry_usuario
    Left = 117
    Top = 264
  end
  object PopupMenu1: TPopupMenu
    Left = 152
    Top = 144
    object Exportaa1: TMenuItem
      Action = modifica
    end
    object Exportaa2: TMenuItem
      Action = exportar
    end
  end
  object qry_permgrant: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'delete from car_perfilusuario'
      'where idusuario = :IDUSUARIO'
      '')
    Left = 328
    Top = 264
    ParamData = <
      item
        DataType = ftInteger
        Name = 'IDUSUARIO'
        ParamType = ptUnknown
      end>
  end
  object qry_perfiles: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'select'
      'IDPERFIL, CLAVE, DESCRI, CIA'
      'from car_perfil where cia = :CIA'
      'order by clave'
      '')
    UpdateObject = uqry_perfiles
    Left = 56
    Top = 232
    ParamData = <
      item
        DataType = ftInteger
        Name = 'CIA'
        ParamType = ptUnknown
      end>
  end
  object uqry_perfiles: TUpdateSQL
    ModifySQL.Strings = (
      'update car_perfil'
      'set'
      '  CLAVE = :CLAVE,'
      '  DESCRI = :DESCRI'
      'where'
      '  IDPERFIL = :OLD_IDPERFIL')
    InsertSQL.Strings = (
      'insert into car_perfil'
      '  (IDPERFIL, CLAVE, DESCRI, CIA)'
      'values'
      '  (:IDPERFIL, :CLAVE, :DESCRI, :CIA)')
    DeleteSQL.Strings = (
      'delete from car_perfil'
      'where'
      '  IDPERFIL = :OLD_IDPERFIL')
    Left = 86
    Top = 232
  end
  object dts_perfiles: TDataSource
    DataSet = qry_perfiles
    Left = 53
    Top = 264
  end
  object qry_eliminapermiperfil: TQuery
    CachedUpdates = True
    DatabaseName = 'manvehi'
    SQL.Strings = (
      'delete from car_perfildet where idperfil = :IDPERFIL'
      '')
    Left = 432
    Top = 264
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'IDPERFIL'
        ParamType = ptUnknown
      end>
  end
end
